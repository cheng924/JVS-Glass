...
class DualBluetoothActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btAdapter?.let {
            DualBluetoothManager.initialize(this, it)
        } ?: run {
            ToastUtils.show(this, "设备不支持蓝牙")
            finish()
            return
        }
        DualBluetoothManager.onServerStarted = {runOnUiThread { tvStatus.text = "服务端初始化成功" }}
        ...
        DualBluetoothManager.onMessageReceived = { msg -> addMessageToHistory("[收到] $msg") }
        ...
        lvDevices.setOnItemClickListener { _, _, pos, _ ->
            ...
            if (hasPermissions(Manifest.permission.BLUETOOTH_CONNECT)) {
                DualBluetoothManager.connectBle(dev)
                DualBluetoothManager.connectClassic(dev)
    ...
    private fun playVoiceMessage(filePath: String) {
        try {
            val mediaPlayer = MediaPlayer().apply {
                ...
                prepare()
                start()
                LogUtils.info("[DualBluetoothActivity] 开始播放语音消息")
                ToastUtils.show(this@DualBluetoothActivity, "正在播放语音消息")
            }
            mediaPlayer.setOnCompletionListener {
                LogUtils.info("[DualBluetoothActivity] 语音消息播放完成")
                it.release()
            }
        } catch (e: IOException) {
